		...
		<div class="card shadow mb-4">
			...
			<form method="get" id="listForm" action="/ks/freesearch">
				<input type="text" id="freesearch" name="freesearch"
					value="${dto.freesearch}" style="width: 300px; height: 40px;"
			...
			<tbody>
				<c:forEach items="${blist}" var="dto">
					<tr>
						...
						<td><a href="/ks/freeboard/${dto.boardno}"> 
	...
	<div style="overflow: hidden;">
			...
			<div style="float: left;">
				<ul class="pagination">
					<li><c:if test="${begin > pageNum }">
							<a href="freesearch?p=${begin-1}&freesearch=${freesearch}" class="page prv"></a> </c:if> 
							<span>«</span>
							</li>
					<li><c:forEach begin="${begin }" end="${end }" var="i">
							<a href="freesearch?p=${i}&freesearch=${search}">${i}</a>
						</c:forEach>
						</li>
					<li><c:if test="${end < totalPages }">
							<a href="freesearch?p=${end+1}&freesearch=${freesearch}" class="page next"></a> </c:if> 
						<span>»</span>
						</li>
				</ul>
			</div>
		...
		<div style="float: right;">
			<button style="margin-right: 0.5em; text-align: right; height: 40px;"
				...
				onclick="location.href='/ks/freeboard'">전체 목록</button>
		</div>
		<!-- 게시글 작성버튼 E -->
	</div>
</div>
