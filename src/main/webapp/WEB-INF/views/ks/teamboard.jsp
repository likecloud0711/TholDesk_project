		...
		<div class="card shadow mb-4">
			...
			<form method="get" id="listForm" action="/ks/teamsearch">
				<input type="text" id="teamsearch" name="teamsearch"
					value="${dto.teamboardListSearch}" style="width: 300px; height: 40px;"
	...
	<div style="overflow: hidden;">
			...
			<div style="float: left;">
				<ul class="pagination">
					<li><c:if test="${begin > pageNum }">
							<a href="teamboard?p=${begin-1}" class="page prv"></a> </c:if> 
							<span>«</span>
							</li>
					<li><c:forEach begin="${begin }" end="${end }" var="i">
							<a href="teamboard?p=${i}">${i}</a>
						</c:forEach>
						</li>
					<li><c:if test="${end < totalPages }">
							<a href="teamboard?p=${end+1 }" class="page next"></a> </c:if> 
						<span>»</span>
						</li>
				</ul>
			</div>
		...
		<div style="float: right;">
			<button style="margin-right: 0.5em; text-align: right; height: 40px;"
				...
				onclick="location.href='/ks/teamwrite'">게시글 작성</button>
		</div>
		<!-- 게시글 작성버튼 E -->
	</div>
</div>
