	...
	<div style="overflow: hidden;">
			...
			<div style="float: left;">
				<ul class="pagination">
					<li><c:if test="${begin > pageNum }">
							<a href="search?p=${begin-1}&search=${search}" class="page prv"></a> </c:if> 
							<span>«</span>
							</li>
					<li><c:forEach begin="${begin }" end="${end }" var="i">
							<a href="search?p=${i}&search=${search}">${i}</a>
						</c:forEach>
						</li>
					<li><c:if test="${end < totalPages }">
							<a href="search?p=${end+1}&search=${search}" class="page next"></a> </c:if> 
						<span>»</span>
						</li>
				</ul>
			</div>
		...
		<div style="float: right;">
			<button style="margin-right: 0.5em; text-align: right; height: 40px;"
				...
				onclick="location.href='/ks/board'">전체 목록</button>
		</div>
		<!-- 게시글 작성버튼 E -->
	</div>
</div>
