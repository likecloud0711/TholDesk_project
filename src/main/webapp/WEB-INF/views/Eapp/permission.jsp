...
<div><jsp:include page="../top.jsp" flush='false' /></div>
		...
		<h3 class="h3 mb-2 font-weight-bold text-gray-800">승인</h3>
	...
	<div class="table-responsive">
		...
		<c:forEach items="${pmlist}" var="pm">
			...
			<tr>
				...
				<td>${pm.empnm } <c:if test="${pm.grade  == 100}">사장</c:if>
						<c:if test="${pm.grade  == 20}">부장</c:if>
						<c:if test="${pm.grade  == 30}">과장</c:if>
						<c:if test="${pm.grade  == 40}">대리</c:if>
						<c:if test="${pm.grade  == 50}">사원</c:if></td>
				<td>${pm.teamnm }</td>
				<td>${pm.draftdt }</td>
